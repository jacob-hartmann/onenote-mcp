...
describe("get-section tool", () => {
  ...
  it("registers with the name 'get-section'", () => {
    expect(mockRegisterTool).toHaveBeenCalledTimes(1);
    expect(mockRegisterTool.mock.calls[0]![0]).toBe("get-section");
  });

  it("returns section details on success", async () => {
    const section = { id: "sec-1", displayName: "My Section" };
    ...
    expect(callArgs.path).toBe("/me/onenote/sections/sec-1");
    expect(callArgs.params).toEqual({ $expand: "parentNotebook" });
  ...
  it("returns error on API failure", async () => {
    ...
    const callback = mockRegisterTool.mock.calls[0]![2] as Function;
    const result = await callback({ sectionId: "missing" }, mockExtra);
